...
export default function Cart() {
  ...
  if (items.length === 0) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-20 text-center">
        ...
        <h1 className="font-display font-800 text-xl text-navy mb-2">Tu carrito está vacío</h1>
        <p className="text-slate-500 mb-6">Todavía no agregaste productos. ¡Dale una vuelta a la tienda!</p>
        <Link to="/tienda" className="bg-brand-500 hover:bg-brand-600 text-white font-semibold px-6 py-3 rounded-lg">
          Ir a la tienda
        </Link>
      </div>
    )
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      <h1 className="font-display font-800 text-2xl text-navy mb-6">Tu carrito</h1>

      <div className="grid md:grid-cols-3 gap-8">
        <div className="md:col-span-2 flex flex-col gap-3">
          {missing > 0 && (
            <div className="bg-brand-50 text-brand-700 text-sm rounded-lg p-3">
              Te faltan $ {missing.toLocaleString('es-AR')} para tener envío gratis 🏍️
            </div>
          )}
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-4 border border-slate-100 rounded-xl p-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-slate-400 text-xs">$ {item.price.toLocaleString('es-AR')} / {item.unit}</p>
              </div>
              ...
              <p className="font-semibold text-navy w-20 text-right shrink-0">
